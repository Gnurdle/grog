  ...
  (if-let [f (config-file)]
    (if (.exists ^java.io.File f)
      (try (edn/read-string (slurp f))
           (catch Exception e (binding [*out* *err*] (println "rss config load error:" (.getMessage e))) {}))
      (binding [*out* *err*] (println "rss config missing: " f) {}))))

(defn- cfg [] (load-config))
(defn- feed-timeout-ms [] (long (or (:timeout-ms (cfg)) 20000)))
(defn- feed-default-limit [] (long (or (:limit (cfg)) 20)))
          ...
          (cond
            ...
            (try
              (let [items (parse-feed body)
                    items (vec (take (or limit (feed-default-limit)) items))]
                (if (empty? items)
