(ns grog-project-search.main
  ...
  Config: ~/.config/grog/project-search.edn
  ...
  (if-let [f (config-file)]
    (if (.exists ^java.io.File f)
      (try (edn/read-string (slurp f))
           (catch Exception e (binding [*out* *err*] (println "project-search config load error:" (.getMessage e))) {}))
      (binding [*out* *err*] (println "project-search config missing: " f) {}))))

(defn- cfg [] (load-config))

(defn- projects-dir ^java.io.File []
  (let [raw (or (get (cfg) :projects-dir) "~/grog-projects")
        home (System/getProperty "user.home")]
    (io/file (str/replace-first raw #"^~(?=/|$)" home))))

(defn- project-root ^java.io.File []
  (let [root (projects-dir)
        explicit (or (get (cfg) :project) nil)
