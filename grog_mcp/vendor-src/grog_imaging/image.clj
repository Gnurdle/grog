...
(defn- imaging-config
  "Load ~/.config/grog/imaging.edn; returns {} when missing/invalid (the file is
  optional, so a missing file is normal and silent)."
  []
  (let [f (imaging-config-file)]
    (if (.exists f)
      (try (edn/read-string (slurp f))
           (catch Exception e (binding [*out* *err*] (println "imaging config load error:" (.getMessage e))) {}))
      {})))

(defn- tessdata-candidate-dirs []
  (let [cfg (:tessdata (imaging-config))
        cfg-dir (when (and cfg (not (str/blank? (str cfg))))
                  (io/file (str/replace-first (str cfg) #"^~(?=/|$)" (System/getProperty "user.home"))))]
    (remove nil?
            (concat
             (when cfg-dir [cfg-dir])
