  ...
  (if-let [f (config-file)]
    (if (.exists ^java.io.File f)
      (try (edn/read-string (slurp f))
           (catch Exception e (binding [*out* *err*] (println "babashka config load error:" (.getMessage e))) {}))
      (binding [*out* *err*] (println "babashka config missing: " f) {}))))

(defn- cfg [] (load-config))

(defn- babashka-command []
  (or (some-> (:cmd (cfg)) str str/trim not-empty)
      "bb"))
