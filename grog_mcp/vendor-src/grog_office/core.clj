...
;; Config is file-based: ~/.config/grog/office.edn
;;   {:bin "soffice" :pdftoppm "pdftoppm"}
...
(defn- config-file []
  (io/file (or (some-> (System/getenv "HOME") str not-empty) "~")
           ".config/grog/office.edn"))

(defn- load-config []
  (if-let [f (config-file)]
    (if (.exists ^java.io.File f)
      (try (edn/read-string (slurp f))
           (catch Exception e (binding [*out* *err*] (println "office config load error:" (.getMessage e))) {}))
      (binding [*out* *err*] (println "office config missing: " f) {}))))

(defn- cfg [] (load-config))
(defn- soffice-bin [] (or (:bin (cfg)) "soffice"))
(defn- pdftoppm-bin [] (or (:pdftoppm (cfg)) "pdftoppm"))
...
(defn render!
  "Render to PDF (or PNG) via headless LibreOffice/`soffice` (override the
  binaries in ~/.config/grog/office.edn). Returns the produced file paths. POI
  cannot render docx itself, so this is the pragmatic ground-truth proxy for
  pixel-diffing."
