  ...
  (if-let [f (config-file)]
    (if (.exists ^java.io.File f)
      (try (edn/read-string (slurp f))
           (catch Exception e (binding [*out* *err*] (println "fetch config load error:" (.getMessage e))) {}))
      (binding [*out* *err*] (println "fetch config missing: " f) {}))))

(defn- cfg [] (load-config))
(defn- fetch-timeout-ms [] (long (or (:timeout-ms (cfg)) 20000)))
(defn- default-max-chars [] (long (or (:max-chars (cfg)) 12000)))
