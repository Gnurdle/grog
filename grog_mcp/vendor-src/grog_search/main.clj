...
(defn- config-file []
  (io/file (or (some-> (System/getenv "HOME") str not-empty) "~")
           ".config/grog/search.edn"))

(defn- load-config []
  (let [f (config-file)]
    (if (.exists f)
      (try (edn/read-string (slurp f))
           (catch Exception e (binding [*out* *err*] (println "search config load error:" (.getMessage e))) {}))
      {})))

(defn- cfg [] (load-config))
(defn- search-timeout-ms [] (long (or (:timeout-ms (cfg)) 20000)))
