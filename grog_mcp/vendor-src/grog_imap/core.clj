...
(defn load-config
  "Load account metadata. With no argument, reads the default accounts file
  ~/.config/grog/imap-accounts.json (the file the MCP server resolves from
  ~/.config/grog/imap.edn). With an argument, accepts a config map, a file
  ...
  ([] (load-config (str (System/getProperty "user.home") "/.config/grog/imap-accounts.json")))
  ([source]
