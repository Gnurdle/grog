...
(defn- imap-config-file ^java.io.File []
  (io/file (or (some-> (System/getenv "HOME") str not-empty) "~")
           ".config/grog/imap.edn"))

(defn- load-imap-config!
  "Load ~/.config/grog/imap.edn. Returns {} when missing or invalid (missing is
  normal — we fall back to the default accounts file)."
  []
  (let [f (imap-config-file)]
    (if (.exists f)
...
(defn- load-config!
  "Load account metadata. Config is file-based: ~/.config/grog/imap.edn
    {:config \"~/.config/grog/imap-accounts.json\"}
  `:config` is a file path (or inline EDN/JSON). When imap.edn is missing or has
  no `:config`, the default accounts file ~/.config/grog/imap-accounts.json is
  used (legacy GROG_IMAP_CONFIG behavior)."
  []
  (let [home (or (System/getenv "HOME") (System/getProperty "user.home"))
        cfg (load-imap-config!)
        src (or (:config cfg) "~/.config/grog/imap-accounts.json")
        src (if (or (str/starts-with? (str/trim src) "{")
                    (str/starts-with? (str/trim src) ":"))
              src
              (str/replace-first src #"^~(?=/|$)" home))
        config (core/load-config src)]
    (when-not (seq (core/accounts config))
      (throw (ex-info (str "IMAP accounts config contains no accounts: " src) {})))
    (reset! config* config)
    (reset! current* nil)
    config))
